...
class InputBubble(NSWindow):
    ...
    def initWithParent_sendCallback_(self, parent_win, send_callback):  # noqa: N802
        ...
        # IMPORTANT: Allow this window to become key and receive input
        ...
        self.contentView().setWantsLayer_(True)
        bg_layer = self.contentView().layer()
        bg_layer.setBackgroundColor_(NSColor.colorWithCalibratedRed_green_blue_alpha_(1.0, 0.8, 0.3, 0.95).CGColor())  # More orange/yellow
        bg_layer.setCornerRadius_(12.0)
        bg_layer.setBorderWidth_(2.0)
        bg_layer.setBorderColor_(NSColor.colorWithCalibratedRed_green_blue_alpha_(0.8, 0.6, 0.2, 1.0).CGColor())

        # text field
        self.text_field = AppKit.NSTextField.alloc().initWithFrame_(((15, 60), (w - 30, 24)))
        self.text_field.setFont_(AppKit.NSFont.systemFontOfSize_(14))  # Slightly larger font
        self.text_field.setBezeled_(True)
        self.text_field.setBezelStyle_(AppKit.NSTextFieldRoundedBezel)
        self.text_field.setFocusRingType_(AppKit.NSFocusRingTypeDefault)  # Show focus ring
        self.text_field.setTarget_(self)
        self.text_field.setAction_("send:")
        self.text_field.setPlaceholderString_("Type your message to Pepe...")  # Add placeholder
        ...
        # Show window and set up focus properly
        self.orderFront_(None)
        self.makeKeyAndOrderFront_(None)
        
        # Use a timer to set focus after the window is fully displayed
        NSTimer.scheduledTimerWithTimeInterval_target_selector_userInfo_repeats_(
            0.1, self, "setFocus:", None, False
        )
    ...
    def setFocus_(self, timer):  # noqa: N802
        """Called by timer to set focus after window is displayed."""
        print("Setting focus to text field...")
        self.makeFirstResponder_(self.text_field)
        success = self.text_field.becomeFirstResponder()
        print(f"Focus set success: {success}")
    ...
    def canBecomeKeyWindow(self):  # noqa: N802
        print("canBecomeKeyWindow called - returning True")
        return True

    def canBecomeMainWindow(self):  # noqa: N802
        print("canBecomeMainWindow called - returning True")
        return True
        
    def acceptsFirstResponder(self):  # noqa: N802
...
class PetDelegate(NSObject):
    def applicationDidFinishLaunching_(self, notification):
        gif_path = os.path.join(
            ...
            "assets",
            "pepe",
            "pepe-smart.gif",  # Using smart pepe as the default/idle animation
        )
    ...
    def show_chat(self):
        ...
        if not os.getenv("OPENAI_API_KEY"):
            print("No OpenAI API key found, prompting user...")
            if not self._prompt_openai_key():
                print("User cancelled API key prompt")
                return  # user cancelled
        else:
            print("OpenAI API key found in environment or file")

        # ---- atomic guard: only continue if we can acquire the lock
        if not self._chat_lock.acquire(blocking=False):
            print("Chat already in progress, ignoring double-click")
            return         # somebody else is already creating / showing chat

        print("show_chat invoked")

        # Use AppleScript to show native input dialog
        import subprocess
        
        applescript = '''
        display dialog "What would you like to say to Pepe? 🐸" ¬
            default answer "" ¬
            with title "Chat with Pepe" ¬
            with icon note ¬
            buttons {"Cancel", "Send"} ¬
            default button "Send"
        '''
        ...
        try:
            print("Showing AppleScript input dialog...")
            result = subprocess.run(['osascript', '-e', applescript], 
                                  capture_output=True, text=True, timeout=60)
            
            print(f"AppleScript result code: {result.returncode}")
            print(f"AppleScript stdout: {result.stdout}")
            print(f"AppleScript stderr: {result.stderr}")
            
            if result.returncode == 0:
                # Parse the result - format is "button returned:Send, text returned:message"
                output = result.stdout.strip()
                if "text returned:" in output:
                    user_msg = output.split("text returned:")[-1].strip()
                    print(f"User message received: '{user_msg}'")
                    ...
                    print("Could not parse AppleScript output")
                    self._chat_lock.release()
            else:
                print("User cancelled AppleScript dialog or error occurred")
                self._chat_lock.release()
            ...
            print(f"Error showing AppleScript dialog: {e}")
            self._chat_lock.release()
    ...
    def ask_gpt_and_respond(self, user_msg: str):
        ...
        try:
            resp = self._http.get(
                ...
                params={"q": user_msg, "format": "json", "no_html": 1, "t": "desktop-pepe"},
                timeout=5,
            )
            data = resp.json()
        ...
        # If the user explicitly asks about pepe price, fetch live price
        price_snippet = ""
        price_intent = False
        try:
            # trigger on any mention of pepe and finance-related words
            if re.search(r"(pepe|pepecoin)", user_msg, re.I):
            ...
            if price_intent:
                ...
                if now - self._price_cache["ts"] < 3600 and self._price_cache["snippet"]:
                    ...
                    resp_p = self._http.get(
                        ...
                        params={"vs_currency": "usd", "ids": "pepe"},
                        timeout=5,
                    )
                    data_p = resp_p.json()
                    if isinstance(data_p, list) and data_p:
                        ...
                        if price is not None:
                            if change is not None:
                                price_snippet = (
                                    f"Pepe live price: ${price:.8f} USD (24h change: {change:+.2f}%). "
                                    "Source: CoinGecko."
                                )
                            else:
                                price_snippet = f"Pepe live price: ${price:.8f} USD (CoinGecko)."
                            # update cache
        ...
        system_prompt = {
            "role": "system",
            "content": (
                "You are Desktop Pepe. Answer very briefly (≤25 words) and append one mood tag "
        ...
        # No input bubble to clean up since we used a modal dialog
    ...
    ANIMATIONS = {
        "idle":      "pepe-smart.gif",        # Using smart pepe as idle
        "happy":     "pepe-silly.gif",        # Silly pepe for happy mood
        "laugh":     "pepe-clown.gif",        # Clown pepe for laughing
        "wow":       "pepe-disappears.gif",   # Disappearing for wow/surprise
        "sad":       "pepe-sad.gif",          # Sad pepe for sad mood
        "thinking":  "pepe-smart.gif",        # Smart pepe for thinking
        "boxing":    "pepe-boxing.gif",       # Boxing pepe for intense/angry
    }

    def set_animation(self, key: str):
        """Load a new GIF and restart the timer."""
        fname = self.ANIMATIONS.get(key, self.ANIMATIONS["idle"])
        path = os.path.join(os.path.dirname(__file__), "assets", "pepe", fname)
    ...
    def _ensure_price_snippet(self, force_fetch: bool = False):
        ...
        try:
            ...
            if isinstance(data, list) and data:
                ...
                if price is not None:
                    snippet = (
                        f"Pepe price: ${price:.8f} USD (24h {change:+.2f}%)."
                        if change is not None else f"Pepe price: ${price:.8f} USD."
                    )
    ...
    def show_price(self):
        """Display current Pepe price in a bubble."""
